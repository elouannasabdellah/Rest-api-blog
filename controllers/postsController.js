...
const { cloudiaryUploadImage, cloudiaryRemoveImage } = require('../utils/cloudinary');
...
module.exports.getAllPostsCtrl = asyncHandler( async(req, res)=>{
    ...
    const POST_PER_PAGE= 3; // chaque page il ya 3 posts
...
module.exports.deletePostCtrl = asyncHandler( async(req,res)=>{
    ...
    if(req.user.isAdmin || req.user.id === post.user.toString() ) {
        ...
        await cloudiaryRemoveImage(post.image.publicId);
...
module.exports.updatePostCtrl= asyncHandler( async(req,res)=>{
    ...
    if(req.user.id !== post.user.toString() ){
        return res.status(403).json({message: "Access denied , you are not the owner the post"});
    }
